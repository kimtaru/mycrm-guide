...
import { CALENDAR_LLM_GUIDE_PARTS } from "./guide-parts";

function buildSampleText() {
  const endpointLines = CALENDAR_LLM_GUIDE_PARTS.map(
    (part) =>
      `- ${part.titleKo}: ${part.hintKo}\ncurl https://www.mycrm-ui.com/document/calendar/${part.partSlug}-llm-guide.md`,
  ).join("\n");

  return `스킬 호출 뒤에 이어지는 사용자 문장을 실제 @mycrm-ui/components Calendar/DatePicker 요청으로 간주하라.
코드를 작성하기 전에 필요한 가이드는 제공된 엔드포인트 리스트 중 적절한 것을 호출(curl)해서 먼저 열람하라.
Calendar와 DatePicker의 실제 동작, props, 기본값, 주의점은 가이드 내용을 우선 신뢰하라.

가이드 엔드포인트
${endpointLines}`;
}

export default function CalendarLlmSkillSample() {
    ...
    <section className="mt-10 overflow-hidden rounded-2xl border border-outline-variant/25 bg-surface-container-lowest shadow-sm">
      ...
      <div className="space-y-5 px-6 py-6">
        <p className="leading-7 text-on-surface-variant">
          아래 샘플은 Skill, Command, 또는 재사용 가능한 지침 블록으로 등록해 사용할 수 있습니다.
          실제 Calendar/DatePicker 요청을 이 프롬프트와 함께 전달하면 에이전트가 필요한 가이드를 먼저 읽고 구현 기준을 맞출 수 있습니다.
        </p>

        <div className="rounded-xl border border-outline-variant/20 bg-surface-container-low px-5 py-4">
          <p className="text-sm font-semibold text-on-surface">사용 개념</p>
          <p className="mt-2 text-sm leading-6 text-on-surface-variant">
            사용 중인 에이전트가 지원하는 방식으로 이 프롬프트를 등록한 뒤,
            실제 사용자 요청을 함께 전달하세요. 어떤 도구는 커맨드 형태 호출을 지원하고,
            어떤 도구는 저장된 프롬프트나 지침 파일 형태를 사용합니다.
          </p>
            ...
            <div className="overflow-x-auto px-4 py-4 font-mono text-sm leading-7 text-slate-100">
              <div className="flex min-w-max items-start gap-3">
                ...
                <code className="whitespace-pre-wrap break-keep text-slate-50">
                  $make-calendar DatePicker에 9월 수요일 비활성화와 행사 marker를 추가해줘
                </code>
              </div>
            </div>
          </div>
